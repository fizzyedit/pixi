...
- `build.zig.zon` pins fizzy's **`sdk/` package** (not the repo root — the root zon owns Velopack and other app-only deps). Locally: `.path = "../../fizzy/sdk"`. For a tagged pin, use the release asset URL `…/releases/download/sdk-vX.Y.Z/fizzy-sdk-vX.Y.Z.tar.gz` (not the git archive of the tag).
